...
if __name__ == '__main__':
    ...
    while input_video.isOpened():
        ...
        if not ret:
            break
        model_img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)).resize((768, 1024))
        ...
        masked_vton_img.save(f'./OOTDiffusion/run/images_output/heygen2/mask_{count}.jpg')
        ...
        image_idx = 0
        for image in images:
            image.save('./OOTDiffusion/run/images_output/heygen2/out_' + model_type + '_' + str(count) + '_'+ str(image_idx) + '.png')
            image_idx += 1
        count += 1
    input_video.release()
