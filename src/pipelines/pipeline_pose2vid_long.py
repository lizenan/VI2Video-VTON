    ...
    def __call__(
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                noise_pred = torch.zeros(
                    (
                        initial_latent.shape[0] * (2 if do_classifier_free_guidance else 1),
                ...
                counter = torch.zeros(
                    (1, 1, initial_latent.shape[2], 1, 1),
                    device=initial_latent.device,
                    dtype=initial_latent.dtype,
                )
                ...
                latents_cat = torch.cat([initial_latent,agnostic_image_latents[:,:,:1],agn_mask[:,:,:1]], dim=1)
                ...
                initial_latent = self.scheduler.step(
                    noise_pred, t, initial_latent, **extra_step_kwargs
                ).prev_sample
                ...
                if i == len(timesteps) - 1 or (
                    ...
                    if callback is not None and i % callback_steps == 0:
                        ...
                        callback(step_idx, t, initial_latent)
            #reference_control_reader.clear()
            #reference_control_writer.clear()
        # repainting
        noise = latents.clone()#[:,:,:1].expand(latents.shape)
        latents = initial_latent.expand(latents.shape)
        latents = self.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            width,
        ...
        latents = self.scheduler.add_noise(
            latents, noise, torch.tensor([noise_timestep])
        )
        """
        noise_timestep = 625

        # Get the noise variance for the chosen timestep
        alpha_prod = self.scheduler.alphas_cumprod[noise_timestep]  # alpha_cumprod is a scalar at this timestep
        beta_t = self.scheduler.betas[noise_timestep]  # beta is the noise variance

        # Add noise according to the DDPM noise schedule formula
        latents = torch.sqrt(alpha_prod) * latents + torch.sqrt(1 - alpha_prod) * noise
